...
class Car(Entity):
    ...
    def compute_steering(self):
        # Steering
        self.rotation_y += self.rotation_speed * 50 * 0.1

        # The car's linear momentum decreases the rotation.
        if self.rotation_speed > 0:
            self.rotation_speed -= self.speed / 6 * 0.1
        elif self.rotation_speed < 0:
            self.rotation_speed += self.speed / 6 * 0.1
        ...
        if self.speed > 0.5 or self.speed < -0.5:
            if held_keys[self.controls[1]] or held_keys["left arrow"]:
                self.rotation_speed -= self.steering_amount * 0.1

                # Turning decreases our speed.
                if self.speed > 1:
                    self.speed -= self.turning_speed * 0.1
                elif self.speed < 0:
                    self.speed += self.turning_speed / 5 * 0.1
    ...
    def update_vertical_position(self, y_ray, movementY):
        ...
        if self.visible:
            if y_ray.distance <= self.scale_y * 1.7 + abs(movementY):
                ...
                self.y += movementY * 50 * 0.1
                self.velocity_y -= 50 * 0.1
    ...
    def update(self):
        ...
        if held_keys[self.controls[0]] or held_keys["up arrow"]:
            ...
            self.driving = False
            if self.speed > 1:
                self.speed -= self.friction * 5 * 0.1
            elif self.speed < -1:
                self.speed += self.friction * 5 * 0.1
        ...
        if held_keys[self.controls[2] or held_keys["down arrow"]]:
            if self.speed > 0:
                self.speed -= self.braking_strenth * 0.1
            else:
                self.speed -= self.acceleration * 0.1
            self.braking = True
        else:
        ...
        if held_keys[self.controls[1]] or held_keys["left arrow"] or held_keys[self.controls[3]] or held_keys["right arrow"]:
            ...
            travelled_dist = abs(self.speed * 0.1)
        ...
        total_dist_to_move = self.speed * 0.1
